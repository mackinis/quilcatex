...
import { AdminData } from './auth-modal';

const formSchema = z.object({
  token: z.string().min(1, { message: 'El token es requerido.' }),
});

type TokenVerificationFormProps = {
  adminData: AdminData;
  onTokenVerified: () => void;
};

export function TokenVerificationForm({ adminData, onTokenVerified }: TokenVerificationFormProps) {
  ...
  const handleResendToken = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Error al reenviar el token.');
      }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
        const response = await fetch('/api/auth/verify-token', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: adminData.email, token: values.token }),
        });

        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || 'Token incorrecto o expirado.');
        }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
        ...
        <div className="flex flex-col space-y-2">
            ...
            {isLoading ? 'Verificando...' : 'Verificar y Finalizar Registro'}
            </Button>
            <Button
                type="button"
                variant="link"
